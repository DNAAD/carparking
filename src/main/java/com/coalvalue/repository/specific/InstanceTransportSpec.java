...
public class InstanceTransportSpec {
    ...
    private InstanceTransportDto bankCardDto;

    public InstanceTransportSpec(InstanceTransportDto bankCardDto) {
        this.bankCardDto = bankCardDto;
    }

    public Specification<InstanceTransport> queryMessagesSpec() {
        ...
        Specification<InstanceTransport> specification = (root, query, cb) -> {
            ...
            if (!StringUtil.isEmpty(bankCardDto.getSearchText())) {
              ...
              //  predicateList.add(cb.or(predicate));
                Predicate predicate_pro = cb.like(root.<String>get("plateNumber"), "%"+ bankCardDto.getSearchText().trim()+"%");
