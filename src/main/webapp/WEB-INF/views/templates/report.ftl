...
-->
    ...
    <script src="${rc.contextPath}/js/jquery.newsTicker.js"></script>
...
<div class=" row" style="padding-bottom: 5px">
    <div class="" style="">
        ...
        <div class="col-lg-6">
            <div id="publicTrendencyContainer" style="width:100%; height: 300px"></div>
            <script type="text/javascript">



                var chart__= Highcharts.chart('publicTrendencyContainer', {
                    chart: {
                        type: 'spline',
                        scrollablePlotArea: {
                            minWidth: 600,
                ...
                req.done(function (data) {
                    data.forEach(function(serie) { // for each row of data, add a series
                        chart__.addSeries(serie, false); // false is to prevent redrawing
                    }.bind(this));
                    chart__.redraw(); // manually redraw
                    chart__.hideLoading(); // s
                ...
                function update() {
                    ...
                    req.done(function (data) {
                        ...
                        for (var i = chart__.series.length-1; i>=0; i--) {
                            chart__.series[i].remove();
                        }
                        for (var y = data.length-1; y >= 0; y--) {
                            chart__.addSeries(data[y]);
                        }
                     //   alert(JSON.stringify(data[0].data[0]));
/*
                      chart__.series[0].update({
                           pointStart: data[0].start,
                            data: data[0].data
